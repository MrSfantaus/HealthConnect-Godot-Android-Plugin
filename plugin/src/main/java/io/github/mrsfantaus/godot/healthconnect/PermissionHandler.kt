...
class PermissionHandler(
    ...
    companion object {
        ...
        private val TYPE_MAP: Map<String, KClass<out Record>> = mapOf(
            ...
            // "BMI" to BodyMassIndexRecord::class, // Missing in this version?
    ...
    fun requestPermissions(permissionsDict: Dictionary) {
        ...
        activity.runOnUiThread {
            try {
                Log.i(TAG, "Launching Health Connect permissions UI via SDK Contract...")
                
                // Use the official SDK contract to generate the intent
                val requestPermissionContext = PermissionController.createRequestPermissionResultContract()
                val intent = requestPermissionContext.createIntent(activity, permissions)
                
                Log.i(TAG, "Intent generated by SDK: $intent")
                activity.startActivityForResult(intent, PERMISSIONS_REQUEST_CODE)
                Log.i(TAG, "startActivityForResult (SDK) called successfully")
            } catch (e: Exception) {
                Log.e(TAG, "SDK contract failed: ${e.message}", e)
                try {
    ...
    fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        // Handled in HealthConnectManager for signal emission
    }
}